	<?php
		$default_site_logo 			= (isset($settings['site_logo'])) 		? $settings['site_logo'] : 'logo.png'; 
		...
		$card_user_name 			= $user_stripe_id 	 = $user_stripe_card_id 	= $user_card_status = $card_last_digits = $card_brand 			= '';
		$exp_year 				= $exp_month 		 = $cvv_code 		= '';
		$user_has_acard   			= 0;
		$settings_stripe_type 		= $settings_stripe_secret_key 		= $settings_stripe_public_key = '';	
			
			
		if(!empty($cmp_auth_id) && !empty($merchant_data))
		{
			$stripe_type 				= (isset($merchant_data['stripe_pay_type']) && !empty($merchant_data['stripe_pay_type']))	? $merchant_data['stripe_pay_type'] : '';
			if(empty($stripe_type))
				$stripe_type			= (isset($settings['stripe_pay_type'])) 	? $settings['stripe_pay_type'] 	: '2';
				
			if($stripe_type == 1){
				$stripe_secret_key		= (isset($merchant_data['stripe_live_secret_key']) && !empty($merchant_data['stripe_live_secret_key']))	? $merchant_data['stripe_live_secret_key'] : '';
				$stripe_public_key		= (isset($merchant_data['stripe_live_public_key']) && !empty($merchant_data['stripe_live_public_key']))	? $merchant_data['stripe_live_public_key'] : '';
			}
			else{
				$stripe_secret_key		= (isset($merchant_data['stripe_sandbox_secret_key']) && !empty($merchant_data['stripe_sandbox_secret_key']))	? $merchant_data['stripe_sandbox_secret_key'] : '';
				$stripe_public_key		= (isset($merchant_data['stripe_sandbox_public_key']) && !empty($merchant_data['stripe_sandbox_public_key']))	? $merchant_data['stripe_sandbox_public_key'] : '';
			}
		}
		else{
			$stripe_type 				= (isset($settings['stripe_pay_type'])) 		? $settings['stripe_pay_type'] 		 : 2;
		...
		$settings_stripe_type 			= (isset($settings['stripe_pay_type'])) 		? $settings['stripe_pay_type'] 		 : 2;
			
		if($settings_stripe_type == 1){
			$settings_stripe_secret_key 	= (isset($settings['stripe_live_secret_key'])) 	? $settings['stripe_live_secret_key'] 	 : '';
			$settings_stripe_public_key 	= (isset($settings['stripe_live_public_key'])) 	? $settings['stripe_live_public_key'] 	 : '';
				
				
			if(empty($stripe_secret_key)) $stripe_secret_key	= $settings_stripe_secret_key;
			if(empty($stripe_public_key)) $stripe_public_key	= $settings_stripe_public_key;
		}
		else{
			$settings_stripe_secret_key 	= (isset($settings['stripe_sandbox_secret_key'])) ? $settings['stripe_sandbox_secret_key'] : '';
			$settings_stripe_public_key 	= (isset($settings['stripe_sandbox_public_key'])) ? $settings['stripe_sandbox_public_key'] : '';
	...
	<script>	
		var flash_msg 			= '<?php echo $flash_message ?>', error_msg = '', flash_message_cont = '<?php echo $flash_message_cont ?>';
		var flash_message_job_id = '<?php echo $flash_message_job_id ?>';
		var flash_message_job 	= '<?php echo $flash_message_job ?>';
		var ondemand_job_id 	= '<?php echo $ondemand_job_id ?>';
		var job_uber_rush_det	= [];
		var all_markers_div 	= [];
		
		//console.log('aa: '+flash_msg+' ab: '+flash_message_cont+' ad: '+ondemand_job_id);
	</script>
	...
	<script>
		...
		var  settings_stripe_public_key	= '<?php echo $settings_stripe_public_key; ?>';
	</script>
						...
						<div class="popup-wrap">
							...
							<div class="popup-form hide" id="job_quote_list_cont">
								<form name="payment-form" data-ajax="false" id="payment-form" action="<?php echo base_url().'make-payment' ?>" method="post">
									...
									<input type="hidden" name="is_ondemand" 		id="is_ondemand" 		value="0" />
									...
									<div id="payment_section" class="hide">
										<div class="">
											...
											<fieldset>
												<label id="card_not_avil_msg" class="error" for=""></label>
													
												<?php
													if($current_class == 'readonly="readonly"')
													{
														echo '<div class="ui-grid-b" id="change_pay_card_type">
														...
														$current_class = $current_class . ' style="display: none"';
													}
												?>
													
												<!-- Card Holder Name -->
												<div class="signup-row smallFont">
													<p type="text" name="cardholdername_ex" id="cardholdername_ex" class="card-holder-name form-controls ex-payment-det"></p>
													<input type="text" name="cardholdername" id="cardholdername" data-role="none" maxlength="70" placeholder="Card Holder Name"  value="" class="card-holder-name form-controls" <?php echo $current_class ?>>
												</div>
													
												<!-- Card Number -->
												<div class="signup-row smallFont">
													<p type="text" name="cardnumber_ex" id="cardnumber_ex" class="card-holder-name form-controls ex-payment-det"></p>
													<input type="text" name="cardnumber" id="cardnumber" data-role="none" id="cardnumber" maxlength="19" placeholder="Card Number" value="" class="card-number form-controls" <?php echo $current_class ?>>
												</div>
													
												<!-- Expiry-->
												
												<div id="show_exp_years" class="ui-grid-b">
													<p type="text" name="expirymonth_ex" id="expirymonth_ex" class="card-holder-name form-controls ui-block-a ex-payment-det"></p>
													<div class="ui-block-b ex-payment-det"><span style="color: #a2a1b8;"> / </span></div>
													<p type="text" name="expyear_ex" id="expyear_ex" class="card-holder-name form-controls ui-block-c ex-payment-det"></p>
												</div>
												
												<div id="new_exp_years" class="signup-row selectForm ui-grid-b" <?php echo $current_class ?>>
													<div class="selectForm ui-block-a year-month-drop">
														<select name="expirymonth" id="expirymonth" data-stripe="exp-month">
													...
													<div class="selectForm ui-block-c year-month-drop">
														<select name="expyear" id="expyear" data-stripe="exp-year">
														</select>
														<a href="javascript:void(0)" class="dropdownA ui-link"></a>
													</div>
												...
												<!-- CVV -->
												<div class="signup-row smallFont">
													<p type="text" name="cvv_ex" id="cvv_ex" class="card-holder-name form-controls ex-payment-det"></p>
													<input type="text" name="cvv" id="cvv" data-role="none" id="cvv" placeholder="Cvv" maxlength="4" class="card-cvc form-controls"  <?php echo $current_class ?> value="">
												</div>
	...
	<input type="button" value="Toggle Visibility 1" onclick="all_uber_point[0].toggle();"></input>
