...
<script>
	function set_fleet_id(fleet_id) {
					
					fleet_id=fleet_id.trim();
					
					document.frm_opt.fleets_id.value = fleet_id;
					document.frm_opt.submit();
	}
	
</script>

<form name="frm_opt" action="<?php echo base_url().'control/manage-fleets/edit'; ?>" method="POST">
	<input type="hidden" name="fleets_id" id="fleets_id" value="">
</form>
	<section id="main-content">
		...
		<?php
			...
			if(isset($flash_message)){
				...
				echo '<i class="icon-ok-sign"></i><strong>Success!</strong> Fleet has been successfully added.';
				echo '</div>';
			    }
			...
			<div class="col-sm-12">
				<section class="panel">
					...
					<div class="panel-body">
						<div class="clearfix">
									   <div class="btn-group">
								<button id="editable-sample_new" class="btn btn-primary" onclick="location.href='<?php echo site_url('control'); ?>/manage-fleets/add';">
											   <a href="javascript:void(0);">Add Fleets<i class="fa fa-plus"></i></a>
										  </button>
				...
				<div id="editable-sample_wrapper" class="dataTables_wrapper form-inline" role="grid">		
					...
					<?php
						...
						?>
								<tr>
									<td><?php echo (isset($fleet['first_name'])) ? ucfirst($fleet['first_name']) : 'N/A';?></td>
									<td><?php echo (isset($fleet['last_name'])) ? ucfirst($fleet['last_name']) : 'N/A';?></td>
									<td><?php echo (isset($fleet['email'])) ? $fleet['email'] : 'N/A';?></td>
									<td><?php echo (isset($fleet['company_address']['address'])) ? $fleet['company_address']['address'] : 'N/A';?></td>
									<td><a href="javascript:void(0)" onclick="set_fleet_id('<?php echo $fleet['_id']; ?>');"><i class="fa fa-pencil"></i></a></td>
									<!--<td><a href="javascript:void(0)" onclick="remove_main('<?php echo base_url().'Data_form_controller/remove_all_cat_field/'.addslashes($fleet['id']) ?>')"><i class="fa fa-trash-o"></i></a></td>-->
								</tr>
						<?php
							}
						}
