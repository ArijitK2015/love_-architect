					...
					<section class="panel">
						...
						<div class="panel-body">
							<div class="form">
								...
								<form class="cmxform form-horizontal " id="user_info" method="post" action="<?php echo base_url(); ?>control/manage-depots/edit" enctype="multipart/form-data">
								<input type="hidden" name="depot_unique_id" id="depot_unique_id" value="<?php echo $depot_details[0]['_id']; ?>" />
									<?php //echo '<pre>'; print_r($depot_details); echo '</pre>';
									...
									//For all fixed mandatory field
										if(count($all_fields_fixed)>0)
										{
											...
											foreach($all_fields_fixed as $fixed)
											{
												...
												//print_r($depot_details[0]);
												if (array_key_exists($field_name,$depot_details[0]))
												{
													$value = $depot_details[0][$field_name];
												}
												else
												{
													$value = '';
												...
												$final_value = (is_array($value)) ? $value['address'] : $value;
									...
									<?php
										...
										$final_country_code			= isset($depot_details[0]['country_code']) ? $depot_details[0]['country_code'] : '';
										$final_phone_code			= isset($depot_details[0]['phone_code']) ? $depot_details[0]['phone_code'] : '';
										$final_phone_no			= isset($depot_details[0]['mobile_no']) ? $depot_details[0]['mobile_no'] : '';
											...
											<div class="col-lg-6">
												<select class="form-control" name="user_timezone" id="user_timezone" required>
													...
													<?php
														...
														if(!empty($tzlists))
														{
															$user_timezone	= (isset($depot_details[0]['user_timezone'])) ? $depot_details[0]['user_timezone'] : $system_timezone;
									...
									<div class="form-group ">
											...
											<div class="col-lg-6">
												<select class="form-control" name="admin_status" id="admin_status">
													<option value="1" <?php echo (isset($depot_details[0]['admin_status']) && $depot_details[0]['admin_status'] == '1') ? 'selected' : ''; ?>>Active</option>
													<option value="0" <?php echo (isset($depot_details[0]['admin_status']) && $depot_details[0]['admin_status'] == '0') ? 'selected' : ''; ?>>Inactive</option>
												</select>
											</div>
									</div>	
