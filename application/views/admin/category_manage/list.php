...
<script>
		function set_page_id(page_id) {
			...
			document.getElementById('frm_opt').submit();
			
		}
</script>
<form id="frm_opt" name="frm_opt" action="<?php echo base_url().'control/category-manage/edit'; ?>" method="POST">
...
<section id="main-content">
	...
	<?php
		...
		if(isset($flash_message)){
			...
			echo'</div>';
	               ...
	               if($flash_message == 'pages_deleted')
		    {
			echo '<div class="alert alert-success">';
			echo '<i class="icon-ok-sign"></i><strong>Success!</strong> Data has been successfully deleted.';
			echo '</div>';
		    }
		    if($flash_message == 'pages_not_deleted'){
			echo'<div class="alert alert-error">';
			echo'<i class="icon-remove-sign"></i><strong>Error!</strong> Data could not be deleted.';        
			echo'</div>';
	...
	?>
        ...
        <div class="row">
		    ...
		    <div class="clearfix">
                                <div class="btn-group">
				    <button id="editable-sample_new" class="btn btn-primary" onclick="location.href='<?php echo site_url('control'); ?>/category-manage/add';">
				...
				<?php
					...
					?>
							<tr>
								...
								?></td>
								<td><a href="javascript:void(0)" onclick="set_page_id('<?php echo strval($row['_id']); ?>');"><i class="fa fa-pencil"></i></a>
								
								<a class="fa fa-trash-o" href="javascript:check_confirm('<?php echo strval($row['_id']); ?>')"></a>
								</td>
 ...
 <script>
	function check_confirm(user_id)
	{
	   ...
	   if (del == true)
	   {
		  window.location.href = '<?php echo site_url('control'); ?>/category-manage/delete/'+user_id;
	   }
	}
