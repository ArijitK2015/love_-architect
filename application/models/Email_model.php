...
class Email_model extends CI_Model {
	...
	*/
	...
	public function send_email($to='', $sub='', $msg='', $cc='', $bcc='', $attachments='', $attachments_name='', $to_name=''){
		...
		$smtp_server 	= (!empty($settings->smtp_server)) 	? $settings->smtp_server 	: 'smtp.gmail.com';
		$smtp_port 	= (!empty($settings->smtp_port)) 		? $settings->smtp_port 		: '25';
		$smtp_username = (!empty($settings->smtp_username)) 	? $settings->smtp_username 	: 'test@testmail.com';
		$smtp_password = (!empty($settings->smtp_password)) 	? $settings->smtp_password 	: 'esolz1234';
		
		$site_email 	= (!empty($settings->system_email)) 	? $settings->system_email 	: 'test@testmail.com';
		$site_name 	= (!empty($settings->site_name)) 		? $settings->site_name 		: 'Esolz Technologies';
