...
Class Mongo_db{
	...
	*/
	...
	{
		require FILEUPLOADPATH.'vendor/autoload.php'; // include Composer's autoloader
	...
	*/
	...
	private function connect()
	{
		...
		try
		{
			...
			$this->connect 	= new MongoDB\Client($dns);
			$this->db 		= $this->connect->{$this->database};
			...
			foreach ($wheres as $wh => $val)
			{
				if ($wh == '_id'){
					if ($val instanceof \MongoDB\BSON\ObjectID || preg_match('/^[a-f\d]{24}$/i', $val)) 
						$this->wheres['_id'] = new MongoDB\BSON\ObjectID($val);
					else
						$this->wheres['_id'] = $val;
				}
				else
	...
	*/
	public function get($collection = "", $do_debug = 0)
	{
		if($do_debug)
		{
			echo '<pre>'; print_r($this->wheres); echo '</pre>';
		}
		...
		try{	
			...
			$documents = 	$this->db->{$collection}->find( $this->wheres, ['limit' => (int) $this->limit, 'skip' => $this->offset, 'sort' => $this->sorts] );
						//->limit((int) $this->limit)
						//->skip((int) $this->offset)
						//->sort($this->sorts);
	...
	*/
	public function count($collection = "") 
	{
		...
		$result   = $this->db->{$collection}->find( $this->wheres, ['limit' => (int) $this->limit, 'skip' => $this->offset] );
	...
	*/
	...
	//Get last insert value of a specific attribute
	public function getNextSequenceValue($collection="", $attribute="")
	{
		if (empty($collection))
		{
			show_error("No Mongo collection specified to remove all indexes from", 500);
		...
		if (empty($attribute))
		{
			show_error("No Mongo attribute specified to remove all indexes from", 500);
		}
