<?php
class Manage_driver_controller extends CI_Controller {
    ...
    */
	public function index()
	{
		...
		if(!empty($this->cmp_auth_id))
		{
			...
			$this->mongo_db->where(array('user_type' => 'driver','merchant_id' => $this->cmp_auth_id));
			$all_details 					= $this->mongo_db->get('site_users');	
		}
		else
		{
			$this->mongo_db->where(array('user_type' => 'driver',));
			$all_details 					= $this->mongo_db->get('site_users');
		}
		...
		$data['data']['all_driver']		= $all_details;

		//$data['data']['all_forms']		= array();
		
		$data['view_link'] = 'admin/drivers/index';
		...
		if($do_have_email)
		{
			...
			if($email_id)
			{
				...
				if($count > 0)
				{
					...
					redirect('control/manage-drivers');
				}
			}
			else
			{
		...
		if($do_insert)
		{
			...
			if($insert)
			{
				...
				if(!empty($email_temp))
				{
					...
					$replace 		= array(base_url().'assets/site/images/logo.png', $to_name, 'Driver', $this->site_name);
	...
	public function add(){
		...
		$this->mongo_db->select('*');
		$this->mongo_db->where(array('form_type' => 'driver'));
		...
		$data['view_link'] = 'admin/drivers/add_drivers';
		$this->load->view('includes/template', $data);
	...
	public function updt()
	{
		...
		$all_existing_menus 			= array();
		$data['data']['all_existing_menus']= $all_existing_menus;
		$driver_id 							= ($this->input->post('drivers_id') && $this->input->post('drivers_id')!='') ? strval($this->input->post('drivers_id')) :'';
		...
		{
			$this->mongo_db->select('*');
			$this->mongo_db->where(array('_id' 	=> $driver_id));
			$this->mongo_db->order_by(array('is_fixed' => 'desc', '_id' => 'asc'));
			$driver_details 					= $this->mongo_db->get('site_users');
			$data['data']['driver_details']	=	$driver_details;
		}
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST' && $driver_id=='')
		{
			$driver_form_id 							= ($this->input->post('driver_unique_id') && $this->input->post('driver_unique_id')!='') ? strval($this->input->post('driver_unique_id')) :'';
			$this->mongo_db->order_by(array('_id' => 'desc'));
			$this->mongo_db->limit(1);
			$get_last_id_arr 			= $this->mongo_db->get('site_users');
			...
			$this->mongo_db->where(array('_id' => $driver_form_id));
			$driver_details 	= $this->mongo_db->get('site_users');
		
			$old_image 		= (isset($driver_details[0]['profile_image'])) ? $driver_details[0]['profile_image'] : '';
					...
					if(!empty($to_up_files))
					{
						...
						foreach($to_up_files as $ufld => $up_file)
						{
							...
							if($file_type_det == 'image')
							{
								...
								if($result)
								{
									$old_data 	= (isset($driver_details[0][$ufld])) ? $driver_details[0][$ufld] : '';
									if($old_data)
									{
		...
		if(!isset($data['data']['driver_details']))
		{
			redirect('control/manage-drivers');
		}
		
		
		$data['view_link'] = 'admin/drivers/edit_drivers';
		$this->load->view('includes/template', $data);
	}
