<?php
class Manage_sizes_controller extends CI_Controller {
	...
	*/
	public function index()
	{
		...
		$all_details 					= $this->mongo_db->get('sizes');
		
		$data['data']['all_sizes']		= $all_details;

		$data['view_link'] = 'admin/sizes/index';
	...
	public function add_sizes()
	{
		...
		$data_to_store['is_fragile']		= strval($this->input->post('is_fragile'));
		...
		$insert	= $this->mongo_db->insert('sizes', $data_to_store);
			
		if($insert)
		{
			$this->session->set_flashdata('flash_message', 'insert_success');
			redirect('control/manage-sizes');
		}
		else
		{
	...
	public function add()
	{
		...
		$data['view_link'] = 'admin/sizes/add_sizes';
		$this->load->view('includes/template', $data);
	...
	public function updt()
	{
		...
		echo $sizes_form_id 				= ($this->input->post('edit_size_id') && $this->input->post('edit_size_id')!='') ? strval($this->input->post('edit_size_id')) :'';
		//if save button was clicked, get the data sent via post
		if ($this->input->server('REQUEST_METHOD') === 'POST' && $sizes_form_id != '')
		{
			$data_to_store		= array();
			...
			$this->mongo_db->where(array('_id' => $sizes_form_id));
			$this->mongo_db->set($data_to_store);
			//$this->mongo_db->update('sizes');
		
			if($this->mongo_db->update('sizes'))
			{
				$this->session->set_flashdata('flash_message', 'size_updated');
				redirect('control/manage-sizes');
			} 
			else
			{
				$this->session->set_flashdata('flash_message', 'size_update_failed');
				redirect('control/manage-sizes');
			}
		}
		else{
			$sizes_id 					= ($this->input->post('size_id') && $this->input->post('size_id')!='') ? strval($this->input->post('size_id')) :'';
			if(empty($sizes_id))
				redirect('control/manage-sizes');
			
			$this->mongo_db->where(array('_id' => $sizes_id));
			$data['data']['sizes_details'] 	= $this->mongo_db->get('sizes'); 
		}
		
		$data['view_link'] = 'admin/sizes/edit_sizes';
		$this->load->view('includes/template', $data);
	}
