...
class Jobs_controllers extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->platform_fee 			= (isset($settings_data[0]['platform_fee'])) ? $settings_data[0]['platform_fee'] : $this->platform_fee;
		...
		if(!empty($this->cmp_details))
		{
			...
			$this->site_logo 			= isset($this->cmp_details['site_logo']) 	? $this->cmp_details['site_logo'] 		: $this->site_logo;
			$this->platform_fee 		= isset($this->cmp_details['platform_fee']) 	? $this->cmp_details['platform_fee'] 	: $this->platform_fee;
		...
		else
		{
			...
			$this->platform_fee 		= $this->settings['platform_fee'];
		}
		...
		$this->data['platform_fee']		= $this->platform_fee;
	...
	public function add_job()
	{
		...
		$data['req_job_details']		= array(
										'description' 				=> isset($_REQUEST['description'])					? $_REQUEST['description'] 	: '',
										'pickup_address' 			=> isset($_REQUEST['pickup_address'])				? $_REQUEST['pickup_address'] : '',
										'pickup_address_lat' 		=> isset($_REQUEST['pickup_address_lat'])			? $_REQUEST['pickup_address_lat'] : '',
										'pickup_address_lng' 		=> isset($_REQUEST['pickup_address_lng'])			? $_REQUEST['pickup_address_lng'] : '',
										'pickup_address_country' 	=> isset($_REQUEST['pickup_address_country'])		? $_REQUEST['pickup_address_country'] : '',
										'pickup_address_country_code' => isset($_REQUEST['pickup_address_country_code'])	? $_REQUEST['pickup_address_country_code'] : '',
										'is_pick_valid'			=> 1,
										'drop_address' 			=> isset($_REQUEST['drop_address'])				? $_REQUEST['drop_address'] 	: '',
										'drop_address_lat' 			=> isset($_REQUEST['drop_address_lat'])				? $_REQUEST['drop_address_lat'] : '',
										'drop_address_lng' 			=> isset($_REQUEST['drop_address_lng'])				? $_REQUEST['drop_address_lng'] : '',
										'drop_address_country' 		=> isset($_REQUEST['drop_address_country'])			? $_REQUEST['drop_address_country']	: '',
										'drop_address_country_code' 	=> isset($_REQUEST['drop_address_country_code'])		? $_REQUEST['drop_address_country_code'] : '',
										'is_drop_valid'			=> 1,
										'size_type' 				=> isset($_REQUEST['size_type'])					? $_REQUEST['size_type'] 	: '',
										'weight' 					=> isset($_REQUEST['weight'])						? $_REQUEST['weight'] 		: '',
										'distance_val' 			=> isset($_REQUEST['distance_val'])				? $_REQUEST['distance_val'] 	: '',
										'width' 					=> isset($_REQUEST['width'])						? $_REQUEST['width'] 		: '',
										'height' 					=> isset($_REQUEST['height'])						? $_REQUEST['height'] 		: '',
										'depth' 					=> isset($_REQUEST['depth'])						? $_REQUEST['depth'] 		: '',
										'add_job_pay'				=> isset($_REQUEST['add_job_pay'])					? $_REQUEST['add_job_pay']	: '',
										
										'uber_rush_quote_id'		=> isset($_REQUEST['uber_rush_quote_id'])			? $_REQUEST['uber_rush_quote_id']	: '',
										'uber_rush_quote_price'		=> isset($_REQUEST['uber_rush_quote_price'])			? $_REQUEST['uber_rush_quote_price']	: '',
										'api_type_cmp'				=> isset($_REQUEST['api_type_cmp'])				? $_REQUEST['api_type_cmp'] 	: ''
									);
	...
	public function submit_job()
	{
		...
		$data_to_store['size']			= (object)array('width' => (float)$this->input->post('size_width'), 'height' => (float)$this->input->post('size_height'), 'depth' => (float)$this->input->post('size_length'));
		...
		$data_to_store['is_ondemand']			= '0';
		$data_to_store['on_demand_user']		= '';
		$data_to_store['on_demand_quote_id']	= '';
		$data_to_store['on_demand_quote_price']	= '';
		$data_to_store['do_pay_now']			= '0';
	...
	{
		//echo 'arijit: '.$this->platform_fee.'<br><br>';
		...
		//check if the user have a credit card id and its avilable
		$this->mongo_db->where(array('user_id' => $user_id, 'card_status' => '1', 'status' => '1'));
		$user_stripe_detail = $this->mongo_db->get('users_stripe_details');
		$user_stripe_det 	= (isset($user_stripe_detail[0]) && !empty($user_stripe_detail[0])) 	 ? $user_stripe_detail[0] 	: array();	
			
			
		if(!empty($this->cmp_auth_id) && !empty($this->merchant_data))
		{
			$stripe_type 				= (isset($this->merchant_data['stripe_pay_type']) && !empty($this->merchant_data['stripe_pay_type']))	? $this->merchant_data['stripe_pay_type'] : '';
			if(empty($stripe_type))
				$stripe_type			= (isset($this->settings['stripe_pay_type'])) 		 ? $this->settings['stripe_pay_type'] 	: '2';
				
			if($stripe_type == 1)
				$stripe_secret_key		= (isset($this->merchant_data['stripe_live_secret_key']) && !empty($this->merchant_data['stripe_live_secret_key']))	? $this->merchant_data['stripe_live_secret_key'] : '';
			else
				$stripe_secret_key		= (isset($this->merchant_data['stripe_sandbox_secret_key']) && !empty($this->merchant_data['stripe_sandbox_secret_key']))	? $this->merchant_data['stripe_sandbox_secret_key'] : '';
		}
		else{
			$stripe_type 				= (isset($this->settings['stripe_pay_type'])) 		 ? $this->settings['stripe_pay_type'] 			: 2;
				
			if($stripe_type == 1)
				$stripe_secret_key 		= (isset($this->settings['stripe_live_secret_key'])) 	 ? $this->settings['stripe_live_secret_key'] 	: '';
			else
				$stripe_secret_key 		= (isset($this->settings['stripe_sandbox_secret_key'])) ? $this->settings['stripe_sandbox_secret_key'] 	: '';
		}
		...
		if(!empty($user_stripe_det))
		{
			$customer_id				= isset($user_stripe_det['stripe_id']) 				 ? $user_stripe_det['stripe_id'] 				: '';
				
			require FILEUPLOADPATH.'assets/stripe/lib/Stripe.php';
			Stripe::setApiKey($stripe_secret_key);
				
			try {
				//check if stripe key available
				$retrieve_det 			= Stripe_Customer::retrieve($customer_id);
				$retrieve_det_arr 		= $retrieve_det->__toArray(true);	
			} catch (Exception $e) {
				$retrieve_det_arr 		= $e->jsonBody;
			}	
				
			if(isset($retrieve_det_arr['id'])) 	$all_content['exist_card_valid'] 			= 1;
			else								$all_content['exist_card_valid'] 			= 0;
		...
		if(!empty($job_id) && ($job_id > 0))
		{
					...
					if(!empty($quote_ids_arr))
					{
						...
						foreach($quote_ids_arr as $k => $quote_id)
						{
							...
							if(!empty($job_quote))
							{
								...
								if($is_ondemand)
									$extra_price					= number_format((($job_price * $this->platform_fee) / 100), 2);
								else
									$extra_price					= number_format((($job_price * $this->platform_fee) / 100), 2);
			...
			else
			{
				...
				if(!empty($job_quote_leg_details))
				{
					foreach($job_quote_leg_details as $j => $job_quote)
					{
						...
						$job_quote['is_ondemand']			= $is_ondemand;
						$job_quote['extra_percent_charge']		= $this->extra_percent_charge;
						$job_quote['platform_fee']			= $this->platform_fee;
						$job_quote['job_total_prices']		= number_format($total_price, 2);
			...
			$all_content['platform_fee'] 			= $this->platform_fee;
			$all_content['user_details'] 			= $user_details;
			$all_content['job_details'] 			= (isset($job_details[0])) 	? $job_details[0] 	: $job_details;
	...
	public function get_leg_co_ordinate_connection_details($job_id, $coordinates)
	{
		...
		if(isset($details[0]) && !empty($details[0]))
		{
			...
			if($is_ondemand)
				$job_prices_extra				= number_format((($job_price * $this->platform_fee) / 100), 2);
			else
				$job_prices_extra				= number_format((($job_price * $this->extra_percent_charge) / 100), 2);
	...
	public function submit_leg()
	{
		...
		if($check_already_submit == 0)
		{
			...
			$start_country_data 	= (@file_get_contents('http://ws.geonames.org/countryCode?lat='.$start_location_lat.'&lng='.$start_location_lng.'&username='.$this->geonames_username.'&type=JSON', false, $context));
			...
			$end_country_data 		= (@file_get_contents('http://ws.geonames.org/countryCode?lat='.$end_location_lat.'&lng='.$end_location_lng.'&username='.$this->geonames_username.'&type=JSON', false, $context));
	...
	public function my_job_quote_list()
	{
		...
		$job_objid 				= trim($this->input->get('job_id'));
		...
		//echo "<pre>"; print_r($data['all_quotes_details']); echo "</pre>"; die;
	...
	public function add_activity_submit()
	{
		...
		$end_country_data 		= (@file_get_contents('http://ws.geonames.org/countryCode?lat='.$this->input->post('event_address_lat').'&lng='.$this->input->post('event_address_lng').'&username='.$this->geonames_username.'&type=JSON', false, $context));
		$end_country_data_arr 	= json_decode($end_country_data);
		$end_country_name 		= isset($end_country_data_arr->countryName) ? $end_country_data_arr->countryName : '';
		$end_country_code 		= isset($end_country_data_arr->countryCode) ? $end_country_data_arr->countryCode : '';
	...
	public function get_uber_current_location()
	{
		$job_id 			= $this->input->get('job_id');
			
		//get last uber activity
		$this->mongo_db->where(array('_id' => strval($job_id)));
		$job_details 		= $this->mongo_db->get('jobs');
		$delivery_id		= (isset($job_details[0]['delivery_id'])) 	? $job_details[0]['delivery_id'] 		: '';
		$return_data_arr	= array('job_det' => array(), 'message' => 'error');
			
		$user_id			= (isset($job_details[0]['user_id'])) 		? $job_details[0]['user_id'] 			: '';	
		$job_status		= (isset($job_details[0]['delivery_status']))? $job_details[0]['delivery_status'] 	: '';
			
		//echo '<pre>'; print_r($job_details); echo '</pre>'; 
			
		if($job_status != 'completed' || $job_status != 'client_canceled' || $job_status != 'unknown')
		{
			if($delivery_id)
			{
				...
				if(isset($result[0]) && trim($result[0])!='' )
				{
					...
					if($token)
					{
						...
						if(is_object($json_array1))	$json_array1 = (array) $json_array1;
							
						//echo '<pre>'; print_r($json_array1); echo '</pre>';	
							
						$status		= (isset($json_array1['status']))	? $json_array1['status']	: '';	
						$courier		= (isset($json_array1['courier']))	? $json_array1['courier']: '';		
							
						//update the job details with the current job status
							
						$data_to_update_job['delivery_status']			= $status;
						...
						if(!empty($status))
						{
							...
							if(!empty($courier))
							{
								...
								$event_courier							= (is_object($event_courier))			? (array) $event_courier : $event_courier;
									
								$event_address							= (isset($event_courier['location'])) 	? $event_courier['location'] 				: array();
								$event_address							= (is_object($event_address))			? (array) $event_address : $event_address;
								...
								$event_address_arr['address']				= '';
								$event_address_arr['lat_str']				= isset($job_details[0]['pickup_address']['lat_str']) 	? strval($job_details[0]['pickup_address']['lat_str']) 		: '';
								$event_address_arr['long_str']			= isset($job_details[0]['pickup_address']['long_str']) ? strval($job_details[0]['pickup_address']['long_str']) 		: '';
								$event_address_arr['country']				= '';
								$event_address_arr['country_code']			= '';
							}
							...
							//echo '<pre>'; print_r($data_to_store11_event); echo '</pre>';	
								
							$insert_e1 							= $this->mongo_db->insert('job_events', $data_to_store11_event);
								
								
							$return_data_arr['job_det']['status']		= $uber_status;	
							$return_data_arr['job_det']['job_id']		= strval($job_id);
							$return_data_arr['job_det']['delivery_id']	= strval($delivery_id);
							$return_data_arr['job_det']['job_pick_lat']	= isset($job_details[0]['pickup_address']['lat_str']) 	? $job_details[0]['pickup_address']['lat_str'] : '';
							$return_data_arr['job_det']['job_pick_lng']	= isset($job_details[0]['pickup_address']['long_str']) ? $job_details[0]['pickup_address']['long_str']: '';
							$return_data_arr['job_det']['job_drop_lat']	= isset($job_details[0]['drop_address']['lat_str']) 	? $job_details[0]['drop_address']['lat_str']   : '';
							$return_data_arr['job_det']['job_drop_lng']	= isset($job_details[0]['drop_address']['long_str']) 	? $job_details[0]['drop_address']['long_str']  : '';
							$return_data_arr['job_det']['job_uber_lat']	= isset($event_address['latitude']) 	? $event_address['latitude'] 			: '';
							$return_data_arr['job_det']['job_uber_lng']	= isset($event_address['longitude']) 	? $event_address['longitude'] 		: '';
								
							$return_data_arr['message']				= 'success';	
								
							echo json_encode($return_data_arr);	
						}
						//else echo json_encode($return_data_arr);
					}
					else echo json_encode($return_data_arr);
				}
				else echo json_encode($return_data_arr);
