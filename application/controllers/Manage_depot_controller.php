    ...
    */
	public function index()
	{
		...
		if(!empty($this->cmp_auth_id))
		{
			...
			$this->mongo_db->where(array('user_type' => 'depot','merchant_id' => $this->cmp_auth_id));
			$all_details 					= $this->mongo_db->get('site_users');	
		}
		else
		{
			$this->mongo_db->where(array('user_type' => 'depot',));
			$all_details 					= $this->mongo_db->get('site_users');
		}
		...
		$data['data']['all_depot']		= $all_details;
		
		//$data['data']['all_forms']		= array();
		
		$data['view_link'] = 'admin/depots/index';
		...
		if($do_have_email)
		{
			...
			if($email_id)
			{
				...
				if($count > 0)
				{
					...
					redirect('control/manage-depots');
				}
			}
			else
			{
		...
		if($do_insert)
		{
			...
			if($insert)
			{
				...
				if(!empty($email_temp))
				{
					...
					$replace 		= array(base_url().'assets/site/images/logo.png', $to_name, 'Depot', $this->site_name);
	...
	public function add(){
		...
		$this->mongo_db->select('*');
		$this->mongo_db->where(array('form_type' => 'depot'));
		...
		$data['view_link'] = 'admin/depots/add_depots';
		$this->load->view('includes/template', $data);
	...
	public function updt()
	{
		...
		$all_existing_menus 			= array();
		$data['data']['all_existing_menus']= $all_existing_menus;
		$depot_id 							= ($this->input->post('depots_id') && $this->input->post('depots_id')!='') ? strval($this->input->post('depots_id')) :'';
		...
		{
			$this->mongo_db->select('*');
			$this->mongo_db->where(array('_id' 	=> $depot_id));
			$this->mongo_db->order_by(array('is_fixed' => 'desc', '_id' => 'asc'));
			$depot_details 					= $this->mongo_db->get('site_users');
			$data['data']['depot_details']	=	$depot_details;
		}
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST' && $depot_id=='')
		{
			$depot_form_id 							= ($this->input->post('depot_unique_id') && $this->input->post('depot_unique_id')!='') ? strval($this->input->post('depot_unique_id')) :'';
			$this->mongo_db->order_by(array('_id' => 'desc'));
			$this->mongo_db->limit(1);
			$get_last_id_arr 			= $this->mongo_db->get('site_users');
			...
			if($do_insert)
			{
				...
				$this->mongo_db->where(array('_id' 	=> $depot_form_id));
		...
		if(!isset($data['data']['depot_details']))
		{
			redirect('control/manage-depots');
		}
		
		
		$data['view_link'] = 'admin/depots/edit_depots';
		$this->load->view('includes/template', $data);
	}
