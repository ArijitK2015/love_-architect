<?php
class Manage_customers_controller extends CI_Controller {
    ...
    */
	public function index()
	{
		...
		if(!empty($this->cmp_auth_id))
		{
			...
			$this->mongo_db->where(array('user_type' => 'customer','merchant_id' => $this->cmp_auth_id));
			$all_details 					= $this->mongo_db->get('site_users');	
		}
		else
		{
			$this->mongo_db->where(array('user_type' => 'customer',));
			$all_details 					= $this->mongo_db->get('site_users');
		}
		...
		$data['data']['all_cust']		= $all_details;

		//$data['data']['all_forms']		= array();
		
		$data['view_link'] = 'admin/customers/index';
		...
		if($do_insert)
		{
			...
			if($insert)
			{
				...
				if(!empty($email_temp))
				{
					...
					$replace 		= array(base_url().'assets/site/images/logo.png', $to_name, 'Customer', $this->site_name);
	...
	public function add(){
		...
		$this->mongo_db->select('*');
		$this->mongo_db->where(array('form_type' => 'customer'));
		...
		$data['view_link'] = 'admin/customers/add_customer';
		$this->load->view('includes/template', $data);
	...
	public function updt()
	{
		...
		$all_existing_menus 			= array();
		$data['data']['all_existing_menus']= $all_existing_menus;
		$cust_id 							= ($this->input->post('cust_id') && $this->input->post('cust_id')!='') ? strval($this->input->post('cust_id')) :'';
		...
		{
			$this->mongo_db->select('*');
			$this->mongo_db->where(array('_id' 	=> $cust_id));
			$this->mongo_db->order_by(array('is_fixed' => 'desc', '_id' => 'asc'));
			$cust_details 					= $this->mongo_db->get('site_users');
			$data['data']['cust_details']	=	$cust_details;
			
			//get customer payment stripe details
			$this->mongo_db->select('*');
			$this->mongo_db->where(array('user_id' 	=> $cust_id));
			$stripe_details 					= $this->mongo_db->get('users_stripe_details');
			$total_stripes_card					= count($stripe_details);
			$data['data']['total_cards']	=	$total_stripes_card;
			$data['data']['stripe_details']	=	$stripe_details;
			
			//get customer payment details
			$this->mongo_db->select('*');
			$this->mongo_db->where(array('user_id' 	=> $cust_id));
			$user_payment_details 					= $this->mongo_db->get('user_payments');
			$total_payments					= count($user_payment_details);
			
			if($total_payments>0)
			{
				foreach($user_payment_details as $pay_keys=>$payments)
				{
					$payment_id= strval($payments['_id']);
					$this->mongo_db->select('*');
					$this->mongo_db->where(array('payment_id' 	=> $payment_id));
					$job_payment_details 					= $this->mongo_db->get('job_approved_quotes');
					$user_payment_details[$pay_keys]['job_id']	= (isset($job_payment_details[0]['job_id'])) ? $job_payment_details[0]['job_id'] : '';
					$user_payment_details[$pay_keys]['job_payment_approve_det']	= (isset($job_payment_details[0])) ? $job_payment_details[0] : array();
				}
			}
			
			$data['data']['total_payments']	=	$total_payments;
			$data['data']['job_payment_details']	=	$user_payment_details;
		}
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST' && $cust_id=='')
		{
			$cust_form_id 							= ($this->input->post('cust_unique_id') && $this->input->post('cust_unique_id')!='') ? strval($this->input->post('cust_unique_id')) :'';
			$this->mongo_db->order_by(array('_id' => 'desc'));
			$this->mongo_db->limit(1);
			$get_last_id_arr 			= $this->mongo_db->get('site_users');
			...
			$this->mongo_db->where(array('_id' => $cust_form_id));
			$customer_details 	= $this->mongo_db->get('site_users');
			
			$old_image 		= (isset($customer_details[0]['profile_image'])) ? $customer_details[0]['profile_image'] : '';
					...
					if(!empty($to_up_files))
					{
						...
						foreach($to_up_files as $ufld => $up_file)
						{
							...
							if($file_type_det == 'image')
							{
								...
								if($result)
								{
									$old_data 	= (isset($customer_details[0][$ufld])) ? $customer_details[0][$ufld] : '';
									if($old_data)
									{
		...
		if(!isset($data['data']['cust_details']))
		{
			redirect('control/manage-customers');
		}
		
		
		$data['view_link'] = 'admin/customers/edit_customer';
		$this->load->view('includes/template', $data);
	}
