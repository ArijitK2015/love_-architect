...
class Dashboard_controllers extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->settings			= (isset($settings[0])) 		? $settings[0] : '';
		...
		$this->client_secret		= (isset($settings[0]['client_secret'])) 			? $settings[0]['client_secret'] 			: '';
		$this->client_id			= (isset($settings[0]['client_id'])) 				? $settings[0]['client_id'] 				: '';
		...
		{
			$this->mongo_db->where(array('cmp_auth_id' => $this->cmp_auth_id));
			$merchant_data_arr 			= $this->mongo_db->get('site_users');
			$this->merchant_data		= (isset($merchant_data_arr[0]))	? $merchant_data_arr[0] 	 : array();
		}
			
		$this->data['merchant_data']		= $this->merchant_data;	
		...
		if($this->cmp_auth_id)
		{
			//get merchant users ids
			$this->mongo_db->where(array('merchant_id' => $this->cmp_auth_id));
			$merchant_users_arr 		= $this->mongo_db->get('site_users');
				
			if(!empty($merchant_users_arr))
			{
				foreach($merchant_users_arr as $a => $merch_user)
					$this->merchant_users[]	= (isset($merch_user['_id']))	? strval($merch_user['_id']) : '';
			}
		}
	...
	public function index()
	{
		...
		//Get terms contents
		if($this->cmp_auth_id)
			$this->mongo_db->where(array('page_alias' => 'terms-conditions', 'merchant_id' => $this->cmp_auth_id));
		else
			$this->mongo_db->where(array('page_alias' => 'terms-conditions', 'merchant_id' => ''));
		...
		$this->data['countries'] 		= $country_name;
		$data['data']					= $this->data;
	...
	public function notifications()
	{
		...
		//Fetch all user menus form database
		$this->mongo_db->where(array('menu_type' => '1', 'status' => '1', 'menu_location' => '1'));
		$this->mongo_db->order_by(array('ord_id' => 'asc'));
		$users_all_menus 				= $this->mongo_db->get('menus');
		$this->data['users_all_menus']	= (isset($users_all_menus)) ? $users_all_menus : array();
		
		//Fetch all countries list form database
		$this->mongo_db->order_by(array('name' => 'asc'));
		$all_country_lists 				= $this->mongo_db->get('countries');
		$this->data['country_lists']	= (isset($all_country_lists)) ? $all_country_lists : array();
		
		//Fetch user selected countries
		$this->mongo_db->where(array('user_id' => $user_id));
		$user_job_country_lists 			= $this->mongo_db->get('user_job_countries');
		$this->data['user_job_countries']= (isset($user_job_country_lists[0])) ? $user_job_country_lists[0] : array();
		
		$this->mongo_db->where(array('user_id' => $user_id));
		$all_user_settings 				= $this->mongo_db->get('user_settings');
		$this->data['user_settings']	= (isset($all_user_settings[0])) ? $all_user_settings[0] : array();
		
		if ($this->input->server('REQUEST_METHOD') === 'POST')
		{
			$new_jobs 		= $this->input->post('new_jobs');
			$update_jobs		= $this->input->post('update_jobs');
			$quote_submit_jobs 	= $this->input->post('quote_submit_jobs');
			$quote_accept_jobs 	= $this->input->post('quote_accept_jobs');
			$exception_reports 	= $this->input->post('exception_reports');
			$tracking_updates 	= $this->input->post('tracking_updates');
			$marketing_emails 	= $this->input->post('marketing_emails');
			
			$is_all_countries	= ($this->input->post('is_all_country')) ? $this->input->post('is_all_country') : '0';
			
			$data_to_store['new_jobs_notification']['email'] 			= (isset($new_jobs[0])) ? $new_jobs[0] : '0';
			$data_to_store['new_jobs_notification']['sms'] 			= (isset($new_jobs[1])) ? $new_jobs[1] : '0';
			
			$data_to_store['update_jobs_notification']['email'] 		= (isset($update_jobs[0])) ? $update_jobs[0] : '0';
			$data_to_store['update_jobs_notification']['sms']			= (isset($update_jobs[1])) ? $update_jobs[1] : '0';
			
			$data_to_store['quote_submit_jobs_notification']['email'] 	= (isset($quote_submit_jobs[0])) ? $quote_submit_jobs[0] : '0';
			$data_to_store['quote_submit_jobs_notification']['sms'] 	= (isset($quote_submit_jobs[1])) ? $quote_submit_jobs[1] : '0';
			
			$data_to_store['quote_accept_jobs_notification']['email'] 	= (isset($quote_accept_jobs[0])) ? $quote_accept_jobs[0] : '0';
			$data_to_store['quote_accept_jobs_notification']['sms'] 	= (isset($quote_accept_jobs[1])) ? $quote_accept_jobs[1] : '0';
			
			$data_to_store['exception_reports_notification']['email']	= (isset($exception_reports[0])) ? $exception_reports[0] : '0';
			$data_to_store['exception_reports_notification']['sms']	= (isset($exception_reports[1])) ? $exception_reports[1] : '0';
			
			$data_to_store['tracking_updates_notification']['email']	= (isset($tracking_updates[0])) ? $tracking_updates[0] : '0';
			$data_to_store['tracking_updates_notification']['sms']		= (isset($tracking_updates[1])) ? $tracking_updates[1] : '0';
			
			$data_to_store['marketing_emails_notification']['email'] 	= (isset($marketing_emails[0])) ? $marketing_emails[0] : '0';
			$data_to_store['marketing_emails_notification']['sms'] 	= (isset($marketing_emails[1])) ? $marketing_emails[1] : '0';
			
			//check if this user have settings data or not
			$this->mongo_db->where(array('user_id' => $user_id));
			$count 	= $this->mongo_db->count('user_settings');
			
			if($count > 0)
			{
				$this->mongo_db->where(array('user_id' => $user_id));
				$this->mongo_db->set($data_to_store);
				$this->mongo_db->update('user_settings');
			}
			else{
				$data_to_store['user_id'] 	= $user_id;
				$insert = $this->mongo_db->insert('user_settings', $data_to_store);
			}
			
			$data_to_store2 	= array('user_phone' => str_replace(' ', '', ($this->input->post('mobile_no'))), 'country_code' => $this->input->post('country_code'), 'phone_code' => $this->input->post('phone_code'));
			
			$this->mongo_db->where(array('_id' => strval($user_id)));
			$this->mongo_db->set($data_to_store2);
			$get1 			= $this->mongo_db->update('site_users');
			
			$data_to_store3	= array('is_all_countries' => strval($is_all_countries), 'countries' => array());
			$this->mongo_db->where(array('user_id' => strval($user_id)));
			$this->mongo_db->set($data_to_store3);
			$get2 			= $this->mongo_db->update('user_job_countries');
				
			if($get)
				$this->session->set_flashdata('flash_message', 'info_updated');
			else
				$this->session->set_flashdata('flash_message', 'info_update_failed');
		...
		$this->data['ptitle'] 			= ($this->site_title) ? 'Notification settings - '.ucfirst($this->site_title) : 'Notification settings';
		$data['data']				= $this->data;
		$data['view_link'] 			= 'site/dashboard/notification_settings';
		$this->load->view('includes/template_site', $data);
	}
	
	public function notifications_countries()
	{
		$user_id = ($this->session->userdata('site_user_objId_hotcargo')) ?  $this->session->userdata('site_user_objId_hotcargo') : 1;
		$this->data['user_id'] 		= $user_id;
		$user_type 	= $this->session->userdata('site_user_type_hotcargo');
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST')
		{
			$user_countries 			= $this->input->post('user_countries');
			$data_to_store['countries']	= $user_countries;
			
			$this->mongo_db->where(array('user_id' => $user_id));
			$count 					= $this->mongo_db->count('user_job_countries');
			...
			if($count > 0){
				...
				$data_to_store['user_id']	= $user_id;
				$insert = $this->mongo_db->insert('user_job_countries', $data_to_store);
		...
		$data['data']					= $this->data;
		$data['view_link'] 			= 'site/dashboard/notification_countries';
		$this->load->view('includes/template_site', $data);
	}
	...
	public function map_search_result()
	{
		...
		if($this->session->userdata('site_is_logged_in'))
		{
			$where_arr 	= $where_gt = $where_lt = $where_lte = $order_arr = $cmp_user_ids = $all_exclude_cmp_user_ids = array();
			$where_in_val 	= array(); 
			...
			if(!empty($this->cmp_auth_id)){
				...
				//Get all users which is not in any merchant since no merchant is is provided
				$this->mongo_db->where_ne('merchant_id', '');
				$all_cmp_users_arr 			= $this->mongo_db->get('site_users');
					
				if(!empty($all_cmp_users_arr)) {
						
					foreach($all_cmp_users_arr as $cmp) $all_exclude_cmp_user_ids[] = (isset($cmp['_id'])) ? strval($cmp['_id']) : '';
				}
				
			}
			//echo '<pre>'; print_r($cmp_user_ids);
			...
			if(!empty($search_type)){
				...
				if(in_array('job', $user_types))
				{
					...
					//Excluding all users which is in any merchant id
					if(!empty($all_exclude_cmp_user_ids))
						$this->mongo_db->where_not_in('user_id', $all_exclude_cmp_user_ids);
					...
					$this->mongo_db->where(array('status' => '1'));
					//$this->mongo_db->where(array('status' => '1', 'is_ondemand' => '1'));
		...
		if(!empty($job_lists))
		{
			...
			foreach($job_lists as $lists)
			{
				...
				if(isset($lists['job_status']) && ($lists['job_status'] == '2'))
				{
					...
					$this->mongo_db->where(array('job_id' => strval($lists['_id']), 'event_type' => 'update_location', 'status' => '1'));
				...
				if(isset($lists['job_status']) && ($lists['job_status'] == '0')){
					$icon = assets_url('site/images/white-job.png'); $job_status_txt = 'New Job'; 			$job_color = '#FFFFFF';
				}
				elseif(isset($lists['job_status']) && ($lists['job_status'] == '1')){
					$icon = assets_url('site/images/blue-job.png'); $job_status_txt = 'Job Quoted on'; 		$job_color = '#032DCD';
				}
				elseif(isset($lists['job_status']) && ($lists['job_status'] == '2')){
					$icon = assets_url('site/images/green-job.png'); $job_status_txt = 'Job in progress'; 	$job_color = '#28F202';
				}
				...
				if(!empty($all_legs))
				{
					...
					{
						$legs_arr[$l]['pick_up']['address'] 	= (isset($legs['start_location']['address'])) ? $legs['start_location']['address'] : '';
				...
				$job_info['leg_icon']						= ($is_job_approved) ? assets_url('site/images/green-job.png') : assets_url('site/images/blue-job.png');
				...
				$job_info['delivery_id']						= (isset($lists['delivery_id'])) 		? $lists['delivery_id'] 		: array();
				$job_info['delivery_status']					= (isset($lists['delivery_status'])) 	? $lists['delivery_status'] 	: array();
				$job_info['delivery']						= (isset($lists['delivery'])) 		? $lists['delivery'] 		: array();
				...
				$all_users['marker'][$point]					= array('type' => '2', "marker_color" => $job_color, 'job_info' => $job_info, 'leg_icon' => assets_url('site/images/blue-job.png'), "pick_up" => array('lat' => $pickup_lat, 'lat_str' => $pickup_lat, 'lng' => $pickup_long, 'lng_str' => $pickup_long, 'icon' => $icon), "drop" => array('lat' => $drop_lat, 'lat_str' => $drop_lat, 'lng' => $drop_long, 'lng_str' => $drop_long, 'icon' => $icon), 'is_job_approved' => $is_job_approved, 'job_has_updated_loc' => $job_has_updated_loc, 'has_legs' => $has_legs, 'all_legs' => $legs_arr);
	...
	public function pre_make_payment()
	{
	...
	public function make_payment()
	{
		...
		$is_ondemand					= (isset($job_details[0]['is_ondemand'])) ? $job_details[0]['is_ondemand'] : 0;
		...
		$this->data['settings'] 			= $settings = $this->sitesetting_model->get_settings();
		$myaccount_data 				= $this->myaccount_model->get_account_data($user_id);
		...
		if($payment_type == 'credit_card')
		{
			...
			{
				try {
					//echo 'arijit: '.$stripe_token.' '.$amount_to_charge; die;
					...
					if(!empty($stripe_token) && ($amount_to_charge > 0))
					{
						...
						if(!empty($customer_det))
						{
							...
							if($customer_id)
							{
								...
								$this->mongo_db->where(array('user_id' => $user_id));
								$this->mongo_db->set($data_to_save);
								...
								{	
									...
									if(empty($error_code))
									{
										...
										//if($pay_id)
										{
											...
											//now as payment is done so get uber rush deliver details
											if($is_ondemand)
											{
												//get user details
												$this->mongo_db->where(array('_id' => strval($user_id)));
												...
												$uber_quote_id			= (isset($job_details[0]['on_demand_quote_id'])) 		? $job_details[0]['on_demand_quote_id'] 	: '';
												$pick_up_addr			= (isset($job_details[0]['pickup_address']['address']))? $job_details[0]['pickup_address']['address'] 	: '';
												$pick_city			= (isset($job_details[0]['pickup_address']['city'])) 	? $job_details[0]['pickup_address']['city'] 	: '';
												$pick_state			= (isset($job_details[0]['pickup_address']['state'])) 	? $job_details[0]['pickup_address']['state'] 	: '';
												$pick_code			= (isset($job_details[0]['pickup_address']['code'])) 	? $job_details[0]['pickup_address']['code'] 	: '';
												$pick_country			= (isset($job_details[0]['pickup_address']['country_code'])) 	? $job_details[0]['pickup_address']['country_code'] 	: '';
													
												$pickup_address_lat		= (isset($job_details[0]['pickup_address']['lat'])) 	? $job_details[0]['pickup_address']['lat'] 	: '';
												$pickup_address_lng		= (isset($job_details[0]['pickup_address']['long'])) 	? $job_details[0]['pickup_address']['long'] 	: '';
													
													
													
													
												$drop_of_addr			= (isset($job_details[0]['drop_address']['address'])) 	? $job_details[0]['drop_address']['address'] 	: '';
												$dropoff_city			= (isset($job_details[0]['drop_address']['city'])) 	? $job_details[0]['drop_address']['city'] 	: '';
												$dropoff_state			= (isset($job_details[0]['drop_address']['state'])) 	? $job_details[0]['drop_address']['state'] 	: '';
												$dropoff_code			= (isset($job_details[0]['drop_address']['code'])) 	? $job_details[0]['drop_address']['code'] 	: '';
												$dropoff_country		= (isset($job_details[0]['drop_address']['country_code'])) 	? $job_details[0]['drop_address']['country_code'] 	: '';
												$dropoff_address_lat	= (isset($job_details[0]['drop_address']['lat'])) 	? $job_details[0]['drop_address']['lat'] 	: '';
												$dropoff_address_lng	= (isset($job_details[0]['drop_address']['long'])) 	? $job_details[0]['drop_address']['long'] 	: '';
													
												$item_width			= (isset($job_details[0]['size']['width'])) 	? $job_details[0]['size']['width'] 	: '';
												$item_height			= (isset($job_details[0]['size']['height'])) ? $job_details[0]['size']['height'] 	: '';
												$item_length			= (isset($job_details[0]['size']['depth'])) 	? $job_details[0]['size']['depth'] 	: '';
													
												$item_weight			= (isset($job_details[0]['weight'])) 		? $job_details[0]['weight'] 		: '';
												$is_fragile			= (isset($job_details[0]['is_fragile'])) 	? $job_details[0]['is_fragile']  	: '';
												...
												if(isset($result[0]) && trim($result[0])!='' )
												{
													...
													$token		= isset($json_decode->access_token) ? $json_decode->access_token : '';
														
													if($token)
													{
														$delivery_details['quote_id']			= $uber_quote_id;
														$delivery_details['order_reference_id']	= '';
																
														$delivery_details['items'][0]			= array(
																						'title'			=> '',
																						'quantity'		=> '1',
																						'width'			=> $item_width,
																						'height'			=> $item_height,
														...
														$delivery_details['pickup']['location']	=  array(
																					'address' 	=> $pick_up_addr,
																					'address_2' 	=> '',
																					'city' 		=> $pick_city,
																					'state' 		=> $pick_state,
														...
														$delivery_details['pickup']['contact']	=  array(
																					'first_name' 	=> (isset($user_details[0]['first_name'])) ? $user_details[0]['first_name'] : '',
																					'last_name' 	=> (isset($user_details[0]['last_name'])) ? $user_details[0]['last_name'] : '',
																					'company_name' => (isset($user_details[0]['company_name'])) ? $user_details[0]['company_name'] : '',
																					'email' 		=> (isset($user_details[0]['email'])) ? $user_details[0]['email'] : '',
																					'phone' 		=> array(
																									    'number' => (isset($user_details[0]['user_phone'])) ? $user_details[0]['user_phone'] : '',
																									    'sms_enabled' 	=> (isset($user_details[0]['user_phone']) && !empty($user_details[0]['user_phone'])) ? true : false
																								   ),
																					'special_instructions' 	=> ''
																				);
															
															
														$delivery_details['dropoff']['location']=  array(
																					'address' 	=> $drop_of_addr,
																					'address_2' 	=> '',
																					'city' 		=> $dropoff_city,
																					'state' 		=> $dropoff_state,
														...
														$delivery_details['dropoff']['contact']	=  array(
																					'first_name' 	=> (isset($user_details[0]['first_name'])) ? $user_details[0]['first_name'] : '',
																					'last_name' 	=> (isset($user_details[0]['last_name'])) ? $user_details[0]['last_name'] : '',
																					'company_name' => (isset($user_details[0]['company_name'])) ? $user_details[0]['company_name'] : '',
																					'email' 		=> (isset($user_details[0]['email'])) ? $user_details[0]['email'] : '',
														...
														$delivery_details['includes_alcohol']		= false;	
															
														$delivery_details_json		= json_encode($delivery_details);
														$ch 						= curl_init('https://api.uber.com/v1/deliveries');
															
														//echo '<pre>'; print_r($delivery_details_json); echo '</pre>';	
														...
														curl_setopt_array($ch, array(
															...
															CURLOPT_POSTFIELDS 		=> $delivery_details_json
														));
														...
														//mail("arijit.modak@esolzmail.com", "My subject", $response);	
															
														if(is_object($json_array))
														{
															$delivery_status 		= isset($json_array->status) 		? $json_array->status 		: '';
															$delivery_id			= isset($json_array->delivery_id)	? $json_array->delivery_id	: '';
															
															$data_to_update_job['delivery_status']		= $delivery_status;
															$data_to_update_job['delivery_id']			= isset($json_array->delivery_id)   	? $json_array->delivery_id : '';
															$data_to_update_job['order_reference_id']	= isset($json_array->order_reference_id)  ? $json_array->order_reference_id : '';
															$data_to_update_job['delivery']['pickup']	= isset($json_array->pickup)    		? $json_array->pickup : array();
															$data_to_update_job['delivery']['dropoff']	= isset($json_array->dropoff) 		? $json_array->dropoff : array();
															$data_to_update_job['delivery']['courier']	= isset($json_array->courier) 		? $json_array->courier : array();
															$data_to_update_job['delivery']['items']	= isset($json_array->items)   		? $json_array->items : array();
															$data_to_update_job['delivery']['created_at']	= isset($json_array->created_at)    ? $json_array->created_at : '';
															$data_to_update_job['delivery']['tracking_url']	= isset($json_array->tracking_url)  ? $json_array->tracking_url : '';
															$data_to_update_job['delivery']['currency_code']	= isset($json_array->currency_code)  ? $json_array->currency_code : '';
														}
														else
														{
															$delivery_status 		= isset($json_array['status']) 	? $json_array['status'] 		: '';
															...
															$data_to_update_job['delivery_id']			= isset($json_array['delivery_id'])   		? $json_array['delivery_id'] : '';
															$data_to_update_job['order_reference_id']	= isset($json_array['order_reference_id'])   ? $json_array['order_reference_id'] : '';
															$data_to_update_job['delivery']['pickup']	= isset($json_array['pickup'])    			? $json_array['created_at'] : array();
															$data_to_update_job['delivery']['dropoff']	= isset($json_array['dropoff']) 			? $json_array['dropoff'] : array();
															$data_to_update_job['delivery']['courier']	= isset($json_array['courier']) 			? $json_array['courier'] : array();
															$data_to_update_job['delivery']['items']	= isset($json_array['items'])   			? $json_array['items'] : array();
															$data_to_update_job['delivery']['created_at']	= isset($json_array['created_at'])    	? $json_array['created_at'] : '';
														...
														//echo 'arijit dev status: '.$delivery_status.'<br>';	
														//echo '<pre>'; print_r($data_to_update_job); echo '</pre>';
															
														if($delivery_status == 'processing')
														{
															$this->mongo_db->where(array('_id' => strval($job_id)));
															$this->mongo_db->set($data_to_update_job);
															$this->mongo_db->update('jobs');
																
															//Get current package location
															$ch1 		= curl_init('https://api.uber.com/v1/deliveries/'.$delivery_id);
															...
															$json_array1	= json_decode($response1);
																
															//echo '<pre>'; print_r($json_array1); echo '</pre>';
															//mail("arijit.modak@esolzmail.com", "My subject", $response1);		
																
															if(is_object($json_array1))
															{
																$courier			= (isset($json_array1->courier)) 			? $json_array1->courier : array();
																$cost_fee			= (isset($json_array1->fee)) 				? $json_array1->fee : '0';
																$uber_status		= (isset($json_array1->status)) 			? $json_array1->status : '';
																$tracking_url		= (isset($json_array1->tracking_url)) 		? $json_array1->tracking_url : '';
																$order_reference_id = (isset($json_array1->order_reference_id)) 	? $json_array1->order_reference_id : '';
																$event_courier		= (isset($json_array1->courier)) 			? $json_array1->courier 	: array();
															}
															else
															{
																$courier			= (isset($json_array1['courier'])) 	? $json_array1['courier'] : array();
															...
															$event_address					= (isset($event_courier['location'])) 	? $event_courier['location'] 	: array();
															$event_address					= is_object($event_address)	? (array) $event_address : array();
															//check if the courier details is avilable then use it or user job start location
															//$courier_lat 	= isset($event_address['latitude']) ? $event_address['latitude'] : '';
															$courier_lat	= $pickup_address_lat;
																
															//$courier_lng 	= isset($event_address['longitude']) ? $event_address['longitude'] : '';
															$courier_lng	= $pickup_address_lng;
																
																
																
															$event_address_arr['address']		= '';
															$event_address_arr['lat']		= isset($courier_lat) ? (float)$courier_lat : '';
																
															$event_address_arr['lat_str']		= isset($courier_lat) ? strval($courier_lat) : '';
																
															$event_address_arr['long']		= isset($courier_lng) ? (float)$courier_lng : '';
																
															$event_address_arr['long_str']	= isset($courier_lng) ? strval($courier_lng) : '';
																
															$event_address_arr['country']		= '';
															$event_address_arr['country_code']	= '';
															...
															$event_courier				= is_object($event_courier)			? (array) $event_courier : array();
															$courier_detail['name']		= (isset($event_courier['name'])) 		? $event_courier['name'] : '';  
															$courier_detail['phone']		= (isset($event_courier['phone'])) 	? $event_courier['phone'] : '';
															$courier_detail['picture_url']= (isset($event_courier['picture_url']))? $event_courier['picture_url'] : '';
															$courier_detail['vehicle']	= (isset($event_courier['vehicle'])) 	? $event_courier['vehicle'] : '';
																
															$data_to_store11_event['job_id'] 			= strval($job_id);
															$data_to_store11_event['user_id']			= strval($user_id);
															$data_to_store11_event['delivery_id']		= $delivery_id;
															$data_to_store11_event['uber_rush']		= '1';
															$data_to_store11_event['status']			= $uber_status;
															$data_to_store11_event['tracking_url']		= $tracking_url;
															$data_to_store11_event['courier_detail']	= $courier_detail;
															$data_to_store11_event['order_reference_id']	= $order_reference_id;
															$data_to_store11_event['event_type'] 		= 'update_location';
															$data_to_store11_event['activity_details'] 	= 'Uber rush current locaton';
															$data_to_store11_event['event_cost'] 		= $cost_fee;
															$data_to_store11_event['event_address'] 	= $event_address_arr;
															$data_to_store11_event['added_on'] 		= date('Y-m-d H:i:s');
															$data_to_store11_event['system_timezone']	= $this->system_timezone;
															$data_to_store11_event['status']			= '1';
															$data_to_store11_event['event_image'] 		= array();
																
															$insert_e1 	= $this->mongo_db->insert('job_events', $data_to_store11_event);
																
																
															//echo '<pre>'; print_r($data_to_store11_event); echo '</pre>';	die;	
															$messages1	= array();
																
															$this->session->set_flashdata('flash_message', 'uber_delivery_started');
														}	
														//else $this->session->set_flashdata('flash_message', 'uber_delivery_failed');	
													}
													else	$this->session->set_flashdata('flash_message', 'uber_auth_failed');
												}
												else	$this->session->set_flashdata('flash_message', 'uber_auth_failed');
											}
										...
										if(isset($result[0]) && trim($result[0])!='' )
										{
											...
											if($token)
											{
												...
												if($delivery_status == 'processing')
												{
													...
													$event_address	= (isset($event_courier['location'])) 	? $event_courier['courier'] 	: array();
													$event_address_arr['address']		= '';
													$event_address_arr['lat']		= isset($event_address['latitude']) ? (float)$event_address['latitude'] : '';
													$event_address_arr['lat_str']		= isset($event_address['latitude']) ? strval($event_address['latitude']) : '';
													$event_address_arr['long']		= isset($event_address['longitude']) ? (float)$event_address['longitude'] : '';
													$event_address_arr['long_str']	= isset($event_address['longitude']) ? strval($event_address['longitude']) : '';
													$event_address_arr['country']		= '';
													$event_address_arr['country_code']	= '';
	...
	public function update_job_location()
	{
		...
		{
			$delivery_id 	= isset($job_details['delivery_id']) ? $job_details['delivery_id'] : '';
		...
		$event_address_arr['lat']		= isset($event_address['latitude']) 	? $event_address['latitude'] : '';
		$event_address_arr['long']		= isset($event_address['longitude']) 	? $event_address['longitude'] : '';
			
		return json_encode($event_address_arr);
	}
		...
		if(!empty($all_jobs))
		{
			foreach($all_jobs as $jobs)
			{
				...
				if($pick_addr_code == '')
				{
					...
					$pick_country_data = (@file_get_contents('http://api.geonames.org/countryCode?lat='.$pick_up_addr_lat.'&lng='.$pick_up_addr_lng.'&username='.$this->geonames_username.'&type=JSON', false, $context));
					...
					$drop_country_data = (@file_get_contents('http://api.geonames.org/countryCode?lat='.$pick_up_addr_lat.'&lng='.$pick_up_addr_lng.'&username='.$this->geonames_username.'&type=JSON', false, $context));
		...
		if(!empty($all_users))
		{
			foreach($all_users as $user)
			{
				...
				if($user_type == 'customer' || $user_type == 'fleet' || $user_type == 'broker' || $user_type == 'depot')
				{
					...
					$company_address_data 	= (@file_get_contents('http://api.geonames.org/countryCode?lat='.$company_address_lat.'&lng='.$company_address_lng.'&username='.$this->geonames_username.'&type=JSON', false, $context));
					...
					$delivery_address_data 		= (@file_get_contents('http://api.geonames.org/countryCode?lat='.$delivery_addr_lat.'&lng='.$delivery_addr_lng.'&username='.$this->geonames_username.'&type=JSON', false, $context));
					...
					$pickup_address_data 		= (@file_get_contents('http://api.geonames.org/countryCode?lat='.$pickup_addr_lat.'&lng='.$pickup_addr_lng.'&username='.$this->geonames_username.'&type=JSON', false, $context));
		...
		if($is_ondemand)
		{
			...
			if(isset($result[0]) && trim($result[0])!='' )
			{
				...
				if($token)
				{
					...
					echo '<pre>'; print_r($delivery_details); echo '</pre>';
					
					$delivery_details_js		= json_encode($delivery_details);
					$ch 						= curl_init('https://api.uber.com/v1/deliveries');
					
					//$array 		= array('delivery_id' => '941e3cad-5ed8-4538-a8d3-2a531b006934');
					...
					curl_setopt_array($ch, array(
						...
						CURLOPT_POSTFIELDS 		=> $delivery_details_js
					));
						
					echo 'a'. $response 				= curl_exec($ch);
					$json_array				= json_decode($response);
					$messages					= array();
