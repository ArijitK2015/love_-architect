<?php
class Manage_fleet_controller extends CI_Controller {
    ...
    */
	public function index()
	{
		...
		if(!empty($this->cmp_auth_id))
		{
			...
			$this->mongo_db->where(array('user_type' => 'fleet','merchant_id' => $this->cmp_auth_id));
			$all_details 					= $this->mongo_db->get('site_users');	
		}
		else
		{
			$this->mongo_db->where(array('user_type' => 'fleet',));
			$all_details 					= $this->mongo_db->get('site_users');
		}
		...
		$data['data']['all_fleet']		= $all_details;

		//$data['data']['all_forms']		= array();
		
		$data['view_link'] = 'admin/fleets/index';
		...
		if($do_have_email)
		{
			...
			if($email_id)
			{
				...
				if($count > 0)
				{
					...
					redirect('control/manage-fleets');
				}
			}
			else
			{
		...
		if($do_insert)
		{
			...
			if($insert)
			{
				...
				if(!empty($email_temp))
				{
					...
					$replace 		= array(base_url().'assets/site/images/logo.png', $to_name, 'Fleet', $this->site_name);
	...
	public function add(){
		...
		$this->mongo_db->select('*');
		$this->mongo_db->where(array('form_type' => 'fleet'));
		...
		$data['view_link'] = 'admin/fleets/add_fleets';
		$this->load->view('includes/template', $data);
	...
	public function updt()
	{
		...
		$all_existing_menus 			= array();
		$data['data']['all_existing_menus']= $all_existing_menus;
		$fleet_id 							= ($this->input->post('fleets_id') && $this->input->post('fleets_id')!='') ? strval($this->input->post('fleets_id')) :'';
		...
		{
			$this->mongo_db->select('*');
			$this->mongo_db->where(array('_id' 	=> $fleet_id));
			$this->mongo_db->order_by(array('is_fixed' => 'desc', '_id' => 'asc'));
			$fleet_details 					= $this->mongo_db->get('site_users');
			$data['data']['fleet_details']	=	$fleet_details;
		}
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST' && $fleet_id=='')
		{
			$fleet_form_id 							= ($this->input->post('fleet_unique_id') && $this->input->post('fleet_unique_id')!='') ? strval($this->input->post('fleet_unique_id')) :'';
			$this->mongo_db->order_by(array('_id' => 'desc'));
			$this->mongo_db->limit(1);
			$get_last_id_arr 			= $this->mongo_db->get('site_users');
			...
			if($do_insert)
			{
				...
				$this->mongo_db->where(array('_id' 	=> $fleet_form_id));
		...
		if(!isset($data['data']['fleet_details']))
		{
			redirect('control/manage-fleets');
		}
		
		
		$data['view_link'] = 'admin/fleets/edit_fleets';
		$this->load->view('includes/template', $data);
	}
